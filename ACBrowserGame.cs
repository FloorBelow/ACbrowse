...
namespace ImGuiNET.SampleProgram.XNA {
    ...
    public class AcBrowserGame : Microsoft.Xna.Framework.Game {
        ...
        public AcBrowserGame() {
            ...
            forgepaths = Directory.EnumerateFiles(@"F:\Games\Assassin's Creed Brotherhood", "*.forge").ToArray<string>();
                    ...
                    if (ImGui.TreeNode(forgenames[forge])) {
                        ...
                        for (int dataFile = 0; dataFile < forges[forge].datafileTable.Length; dataFile++) {
                            if (ImGui.TreeNode(forges[forge].datafileTable[dataFile].name)) {
                                if (files[forge][dataFile] is null) files[forge][dataFile] = forges[forge].OpenDatafile(dataFile, null, Forge.ReadResourceType.Empty);
                                ImGui.BeginTable("files", 2);
                                ImGui.TableSetupColumn("Type", ImGuiTableColumnFlags.WidthFixed); //TODO WIDTH BUGGING OUT FOR SOME REASON
                                ...
                                for (int file = 0; file < files[forge][dataFile].Length; file++) {
                                    ...
                                    if (ImGui.BeginPopupContextItem(files[forge][dataFile][file].fileID.ToString())) {
                                        if (ImGui.Selectable("Export")) ExportFile(forge, dataFile, file);
                                        ImGui.Text(files[forge][dataFile][file].name);
                                        ImGui.EndPopup();
        ...
        //TODO this should be in acsharp somehow
        void ExportFile(int forge, int datafile, int file) {
            ForgeFile[] files = forges[forge].OpenDatafile(datafile, null, Forge.ReadResourceType.Raw);
            File.WriteAllBytes($"{files[file].name}.{fileTypes[files[file].fileType]}.ac2", ((ResourceRawData)files[file].resource).data);
        }
    }
